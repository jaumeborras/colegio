  ...
  return (
    <footer ref={footerRef} className="relative overflow-hidden" style={{ backgroundColor: "#001440" }}>
      ...
      <div className="relative max-w-screen-xl mx-auto px-6 py-14 lg:py-16">
        <div className="grid grid-cols-2 md:grid-cols-[1.1fr_0.7fr_0.7fr_3.5fr] gap-4 md:gap-8 items-start">

          {/* Col 1: datos del colegio */}
          <div className="order-1 md:order-none">
            <p className="text-sm md:text-lg font-bold text-white leading-snug mb-3 md:mb-4">
              {t("footer.school", lang)}
            </p>
            <div className="space-y-0.5 text-xs md:text-base text-white leading-relaxed mb-4 md:mb-5">
              <p>{t("footer.hours", lang)}</p>
              <p>Av. Picasso, 21</p>
              <p>07014 Palma de Mallorca</p>
              <p>Islas Baleares — España</p>
            </div>
            <a href="tel:971220575" className="inline-flex items-center gap-2 text-xs md:text-base font-bold text-white hover:text-white/75 transition-colors mb-4 md:mb-5">
            ...
            <div className="flex gap-3 md:gap-5 items-center">
              ...
              <a href="https://www.youtube.com/@sancatv1252" target="_blank" rel="noopener noreferrer" aria-label="YouTube" className="text-white hover:opacity-60 transition-opacity">
                <svg className="w-6 h-6" fill="currentColor" viewBox="0 0 24 24">
          ...
          {/* Col 2 */}
          <div className="order-3 md:order-none md:pt-[38px]">
            <ul className="space-y-2 md:space-y-3">
              <li><Link href="/admisiones" className="text-xs md:text-base text-white hover:opacity-70 transition-opacity">{t("nav.admissions", lang)}</Link></li>
              <li><Link href="/comedor" className="text-xs md:text-base text-white hover:opacity-70 transition-opacity">{t("cafeteria.title", lang)}</Link></li>
              <li><Link href="/extraescolares" className="text-xs md:text-base text-white hover:opacity-70 transition-opacity">{t("extra.title", lang)}</Link></li>
              <li><a href="https://web2.alexiaedu.com/ACWeb/LogOn.aspx?key=bhaA17N5NZc%3d" target="_blank" rel="noopener noreferrer" className="text-xs md:text-base text-white hover:opacity-70 transition-opacity">Alexia (familias)</a></li>
            </ul>
          </div>

          {/* Col 3 */}
          <div className="order-4 md:order-none md:pt-[38px]">
            <ul className="space-y-2 md:space-y-3">
              <li><a href="https://colegiosancayetano.managebac.com/" target="_blank" rel="noopener noreferrer" className="text-xs md:text-base text-white hover:opacity-70 transition-opacity">ManageBac (IB)</a></li>
              <li><a href="https://biblioteca.colegiosancayetano.com/" target="_blank" rel="noopener noreferrer" className="text-xs md:text-base text-white hover:opacity-70 transition-opacity">{t("stage.library", lang)}</a></li>
              <li><Link href="/contacto" className="text-xs md:text-base text-white hover:opacity-70 transition-opacity">{t("contact.title", lang)}</Link></li>
            </ul>
          </div>

          {/* Col 4: Logo grande */}
          <div className="order-2 md:order-none flex justify-center items-center md:pl-12">
            <Link href="/">
              <Image
