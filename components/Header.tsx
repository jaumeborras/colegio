...
function MegaDropdown({ item, description, image, onEnter, onLeave, isActive }: {
  ...
  return (
    ...
    >
      <button className={`flex items-center gap-1 px-3 py-2 text-base font-semibold transition-colors rounded-md ${
...
function NavDropdown({ item }: { item: NavItem }) {
  ...
  if (!hasChildren) {
    return (
      <Link
        href={href}
        {...linkProps}
        className="px-3 py-2 text-base font-semibold text-white/90 hover:text-white hover:bg-white/15 transition-colors rounded-md"
      >
        {item.label}
      </Link>
    )
  ...
  return (
    <div className="relative" onMouseEnter={enter} onMouseLeave={leave}>
      <button className="flex items-center gap-1 px-3 py-2 text-base font-semibold text-white/90 hover:text-white hover:bg-white/15 transition-colors rounded-md">
        {item.label}
...
function LangDropdown() {
  ...
  return (
    <div className="relative ml-1" onMouseEnter={enter} onMouseLeave={leave}>
      <button className="flex items-center gap-1.5 px-3 py-2 text-base font-semibold text-white/90 hover:text-white hover:bg-white/15 transition-colors rounded-md">
            ...
            {getSecondaryNav(lang).slice(1).map((item) => (
              <React.Fragment key={item.label}>
                ...
                <Link
                  ...
                  className={item.cta
                    ? "px-4 py-1.5 text-base font-semibold text-white rounded-full transition-colors"
                    : "px-3 py-2 text-base font-semibold text-white/90 hover:text-white hover:bg-white/15 transition-colors rounded-md"
                  }
                  style={item.cta ? { background: "var(--accent)", boxShadow: "0 0 0 2px rgba(255,255,255,0.35)" } : undefined}
                >
                  {item.label}
                </Link>
              </React.Fragment>
